...
class StrikerAugmentedMixScaleEnv(StrikerEnv):
    ...
    def _get_state(self):
        ...
        state = np.array([
                    ...
                    vel_puck.y,
                    # distance error
                    self.SCALE*wall_dist_S,
                    self.SCALE*wall_dist_E,
                    self.SCALE*wall_dist_N,
                    self.SCALE*wall_dist_W,
                    ], dtype=np.float32)
        assert len(state)==self.dim_observation
        return state
...
class StrikerAugmentedConstantEnv(StrikerEnv):
...
class StrikerTargetEnv(StrikerEnv):
    ...
    def _get_state(self):
        ...
        state = np.array([
                    ...
                    vel_puck.y,
                    # distance error 
                    100*(target_x-puck_x), 100*(target_y-puck_y)
                    ], dtype=np.float32)
        assert len(state)==self.dim_observation
        return state
