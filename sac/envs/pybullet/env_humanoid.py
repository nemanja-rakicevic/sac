...
from pybullet_envs.gym_locomotion_envs import  WalkerBaseBulletEnv, Humanoid
...
class HumanoidEnv(WalkerBaseBulletEnv):
    ...
    def _get_info_dict(self, obs=None):
        ...
        contact_info = obs[np.array([43, 42])] if obs is not None else []
        velocity_info = self.unwrapped.robot_body.speed()
        rel_angle = \
            self.unwrapped.jdict['right_hip_x'].current_relative_position()[0]\
            - self.unwrapped.jdict['left_hip_x'].current_relative_position()[0]
